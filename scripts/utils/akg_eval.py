...
def _remote_host_for_url(worker_url: str) -> Optional[Tuple[str, str]]:
    ...
    if port is None:
        return None
    # workspace_autoresearch/config.yaml is the canonical place; the worker CLI reads the same yaml from cwd/config.yaml.
    import yaml
...
def _make_reconnect_callback(worker_url: str):
    """CA has no import-compatible AKG remote_dispatch module.

    Keep the formal verify/profile path aligned with WA and disable only
    the optional ssh-tunnel auto-reconnect callback. The caller still
    registers the same RemoteWorker protocol; transient reconnect just
    fails fast instead of attempting CA-specific tunnel repair here.
    """
    return None
...
async def _acquire_worker(backend: str, arch: str, device_ids: list[int],
    ...
    from eval.worker.manager import (
...
def _resolve_eval_timeout_s(task_dir: str, config) -> int:
    ...
    every input group, so the wall-clock cap is N x per-shape budget -    otherwise a 30-case op with a 600s per-shape budget hits the worker
...
def _build_verifier(task_dir: str, config, ref_code: str, backend: str,
    ...
    # TaskConfig -mirroring CA's `formal eval` -> `settings.
...
async def _eval_async(task_dir: str, config, device_id: Any,
    ...
    try:
        ...
        try:
            # Profile HTTP read timeout = ``eval_timeout x num_cases``
