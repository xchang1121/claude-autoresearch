...
async def _profile_kernelbench_baseline(
    ...
    try:
        ...
        from eval.worker.manager import get_worker_manager
...
async def _prepare_sol_profile(worker, verifier, profile_dir, warmup_times, run_times, timeout):
    ...
    from eval.sol_verifier import PROF_SOL_BASE_TEMPLATE_PATH
    from eval.adapters.factory import get_framework_adapter, get_backend_adapter
...
async def _prepare_cann_profile(worker, verifier, profile_dir, warmup_times, run_times, timeout):
    ...
    from eval.cann_verifier import PROF_CANN_BASE_TEMPLATE_PATH
    from eval.adapters.factory import get_framework_adapter, get_backend_adapter
    from eval import get_project_root
    from jinja2 import Template
    import yaml
