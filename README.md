...
`verify + profile_gen`。两边各自写 sidecar，最后合并成
`eval_result.json`。这样 kernel 把 NPU device 搞 hang / 子进程被
SIGKILL,也不会影响 ref 数据落盘——之前合并在一个子进程时,
seed kernel 一崩 baseline 永远 `None` 就是这个 bug。verify 和
profile_gen 仍在同一个 kernel 子进程里跑,JIT/autotune cache 依旧
warm。stick baseline 命中时跳过 ref pass。单一 endpoint:
`POST /api/v1/run`。
