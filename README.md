...
scaffold 自动跑 baseline：seed PASS → phase 直接进 PLAN；seed FAIL → phase
也直接进 PLAN，第一批 plan items 用于改写 seed kernel。
...
Reference 输出由 worker 端**每轮在 device 上重算**（不再缓存到 `/tmp`）：

1. 加载 `Model(*get_init_inputs())`，按 case 跑 `Model(*get_inputs())`
   拿 ref 输出
2. 跑 `ModelNew.forward`，按 case 与 ref `torch.allclose` 比对，输出
   `max_abs` / `max_rel` / `bad_elems(%)`

容差固定在 [`utils/correctness.py`](.autoresearch/scripts/utils/correctness.py)：
`DEFAULT_ATOL = DEFAULT_RTOL = 1e-2`。`task.yaml` 不再有 `correctness_atol /
correctness_rtol` 字段，要调精度直接改 `correctness.py` 这一个文件。
