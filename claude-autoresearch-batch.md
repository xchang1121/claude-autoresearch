...
# ── 3. 起 worker daemon（持久占卡，所有 op 串行提交，不抢资源）
...
Tier 1 静态检查（每个 op 独立 subprocess，秒级）：
...
for each pending op:
  起 claude --print  (cwd = repo 根)
    └→ Claude 跑 /autoresearch → scaffold → export AR_TASK_DIR
...
`export AR_TASK_DIR="<path>"` —— 这条 export 激活 hook 链，没跑下游全废。
...
### Tier 2 精度预检（需要 worker 在线）

`prepare.py` 默认只跑 Tier 1（静态）。要在跑 `/autoresearch` 前实跑 ref vs
kernel 一遍：
...
| torch / torch_npu | Tier 2 verify + `/autoresearch` 实跑要；Tier 1 不要 |
...
`ar_cli.py worker --status --port 9111` 看状态，没起就 `--start --bg`。或者
改用 `--devices N` 走 in-process eval。

### phase 卡死、`--retry-errored` 也救不回来

模型偶尔漏跑 `export AR_TASK_DIR`，导致 hook 链没激活。`run.py` 的 prompt
...
### Tier 2 verify 全报 `ModuleNotFoundError: triton`

worker daemon 用了错的 conda env。`source` 进对的 env 再起 daemon。Tier 1
import 失败也是同样原因。
